...
const parseGDLCode = (gdlCode) => {
	...
	// в каждой строке произведем замену координат на новые с учетом x и y: первое число отнимем от x(положительного), второе число отнимем от y(отрицательного). Параллельно округлим до 2 знаков после запятой
...
const createClustersOnColumn = (cornerData) => {	//	// Создание объектов кластеров колонок
	const clusters = [];
	const step = 10;
...
const processData = () => {
	...
	// fs.writeFileSync(devDataFilePath, JSON.stringify(coordsAndHeightLines), "utf8");

	// завершение работы
	// process.exit(1);

	// 
	// const dataLines = getDataFromFile(inputDataFilePath); //	Данные из таблицы вида: 
	...
	// сохраним в файл devDataFilePath
	// fs.writeFileSync(devDataFilePath, JSON.stringify(coordsAndHeightData), "utf8");
	...
	// console.log(`количество точек: ${cellCornerData.length}`);
	// сохраним в файл devDataFilePath
	// fs.writeFileSync(devDataFilePath, JSON.stringify(cellCornerData), "utf8");
